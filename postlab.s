...
PSECT udata_shr ; Variables en la memoria RAM compartida entre bancos
    ...
    pos_timer: DS 1	; 1 byte reservado (Variable para almacenar la cantidad de segundos transcurridos en el contador del timer0)
    tens_count: DS 1	; 1 byte reservado (variable para almacenar la cantidad de decenas en el contador)
...
push:
    ...
    MOVWF STATUS_TEMP
isr:
    btfsc T0IF			;revisamos las banderas de las interrupciones y ejecutamos subrutinas de interrupción en respuesta
    call int_T0
    call limit_checkD
    btfsc   RBIF
    call int_iocb
...
push_limit_checkA:
    BTFSS PORTA, 4	    ;chequeamos si el quinto bit de PORTA está en 1
    goto $+11               ;NO: salimos de la subrutina
    BTFSS PORTA, 7	    ;SÍ, chequeamos si el octavo bit de PORTA está en 1
...
int_T0:
    ...
    INCF pos_timer	  ; Incrementamos nuestra variable para posicionar la cantidad de segundos
    restart_tmr0
    return
...
limit_checkD:
    BTFSS pos_timer, 4	    ;chequeamos si el quinto bit de PORTD está en 1
    goto $+11               ;NO: salimos de la subrutina
    BTFSS pos_timer, 7	    ;SÍ, chequeamos si el octavo bit de PORTD está en 1
    goto $+2		    ;NO: Limpiamos el puerto 
    goto $+3                ;SÍ: Limpiamos el puerto y seteamos el nibble low
    CLRF pos_timer		    
    goto $+6 
...
main:			    
     MOVLW 10		    ; enviamos un valor inicial al counter
    MOVWF counter   
...
loop:
    call units_display	    ;constantemente enviamos el valor de pos_timer al display de unidades
    call tens_display	    ;constantemente enviamos el valor de tens_count al display de decenas
    goto loop
;-------------------------------------------------------------------------------
;subrutinas
;-------------------------------------------------------------------------------
...
units_display:
    ...
    MOVF pos_timer, W	    ; llevamos el valor de pos_timer a W      
    call table		    ; llamamos a la tabla para devolver un valor hexadecimal a W
    MOVWF PORTC		    ; llevamos el valor de W al PORTC
    call units_check
    return

units_check:
    MOVLW 10		    ; movemos la literal 10 a W
    SUBWF pos_timer, W	    ; le restamos el valor del pos_timer a W y lo almacenamos en W
    BTFSS STATUS, 2	    ; verificamos si el resultado de la resta activa la bandera Z de STATUS
    goto $+2		    ; NO: salimos de la subrutina
    call units_reset	    ; SÍ: reiniciamos el valor del pos_timer con la subrutina units_reset
    return
  
units_reset:
    BCF STATUS, 2	    ; limpiamos la bandera Z de STATUS
    CLRF pos_timer	    ; reseteamos el pos_timer
    call tens_check	    ; llamamos a incrementar las decenas en una unidad
    return

tens_display:
    
    MOVF tens_count, W	    ; llevamos el valor de tens_coubt a W 
    call table		    ; llamamos a la tabla para devolver una valor hexadecimal a W
    MOVWF PORTD		    ; llevamos el valor de W a PORTD
    return

tens_check:
    INCF tens_count	    ; Incrementamos las decenas
    MOVLW 6		    ; Movemos la literal 6 a W
    SUBWF tens_count, W	    ; le restamos el valor de tens_count a W y lo almacenamos en W
    BTFSS STATUS, 2	    ; verificamos si el resultado de la resta activa la bandera Z de STATUS
    goto $+2		    ; NO: salimos de la subrutina
    call tens_reset	    ; SÍ: llamamos a resetear las decenas
    return

tens_reset:
    BCF STATUS, 2	    ; limpiamos la bandera de Z de STATUS
    CLRF tens_count	    ; limpiamos las decenas
    return
END